...
namespace Windows.UI.Xaml.Controls
{
	...
	public partial class MenuFlyoutSubItem : MenuFlyoutItemBase, ISubMenuOwner
	{
		...
		Transition m_tpMenuPopupThemeTransition;
	   ...
	   ForwardPresenterProperties(
			...
			MenuFlyoutPresenter pSubMenuFlyoutPresenter)
		{
			...
			bool isTextScaleFactorEnabled = true;
			...
			Debug.Assert(pOwnerMenuFlyout != null && pParentMenuFlyoutPresenter != null && pSubMenuFlyoutPresenter != null);
			...
			isTextScaleFactorEnabled = pParentMenuFlyoutPresenter.IsTextScaleFactorEnabledInternal;
	   ...
	   EnsureCloseExistingSubItems()
		{
			...
			if (spParentPresenter != null)
			{
				...
				openedSubPresenter = spParentPresenter.SubPresenter;
		...
		CreateSubPresenter()
		{
			...
			(spPresenter as MenuFlyoutPresenter).Owner = this;

			return spPresenter;
		}

		void
	   UpdateParentOwner(MenuFlyoutPresenter parentMenuFlyoutPresenter)
		{
			...
			if (parentPresenter != null)
			{
				...
				parentSubMenuOwner = (parentPresenter as MenuFlyoutPresenter).Owner;
				...
				{
					ParentOwner = parentSubMenuOwner;
				}
			}
		}
	   ...
	   SetIsOpen(
		bool isOpen)
		{
			...
			if (isOpen != isOpened)
			{
				(m_tpPresenter as MenuFlyoutPresenter).Owner = isOpen ? this : null;
				...
				if (parentPresenter != null)
				{
					(parentPresenter as MenuFlyoutPresenter).SubPresenter = isOpen ? m_tpPresenter as MenuFlyoutPresenter : null;

					IMenu owningMenu;
					owningMenu = parentPresenter.OwningMenu;

					if (owningMenu != null)
					{
						(m_tpPresenter as MenuFlyoutPresenter.OwningMenu = isOpen ? owningMenu : null);
					}
				...
				if (isOpen)
				{
					DependencyObject spPresenterAsDO = m_tpPresenter;
	   ...
	   Close()
		...
		void
	   ChangeVisualState(bool bUseTransitions)
		{
			...
			shouldBeNarrow = GetShouldBeNarrow();
		...
		SizeChangedEventArgs args)
		{
			...
			openedLength = (m_tpPresenter as Control).ActualHeight;
	   ...
	   ClearStateFlags()
		...
		void
	   OnIsEnabledChanged(IsEnabledChangedEventArgs* args)
		{
#if MFSI_DEBUG
	   ...
	   OnVisibilityChanged()
		...
		ISubMenuOwner ISubMenuOwner.ParentOwner
		{
			...
			set => m_wrParentOwner.Target = value;
		}
		...
		void ISubMenuOwner.PrepareSubMenu()
		{
			...
			Debug.Assert(m_tpPopup != null);
			Debug.Assert(m_tpPresenter != null);
		}
		...
		void ISubMenuOwner.OpenSubMenu(Point position)
		{
			...
			{
				IMenu owningMenu;
				owningMenu = parentMenuFlyoutPresenter.OwningMenu;
				(m_tpPresenter as MenuFlyoutPresenter).OwningMenu = owningMenu;
